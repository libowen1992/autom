...
for node in ${NODES[@]}
    do
      ...
      echo "-- ${node} 停止服务"
      ssh -q -o StrictHostKeyChecking=no -p ${PORT} ${USER}@${node} "source /etc/profile && ${STOP_CMD}"
      echo "-- ${node} 替换新的包"
      ssh -q -o StrictHostKeyChecking=no -p ${PORT} ${USER}@${node} "cd ${APP_PKG_PATH} && rm -fr ${APP_PKG_NAME} && mv ${REMOTE_TMP_DIR}/${APP_PKG_NAME} ."
      echo "-- ${node} 启动服务"
